...
class RankerApp:
    ...
    def build_ui(self):
        ...
        # ----------------------------------------------------
        # Adaptive strategy presets
        # ----------------------------------------------------

        preset_frame = ttk.Frame(outer)
        preset_frame.pack(fill="x", pady=(0, 12))

        ttk.Label(preset_frame, text="Strategy:").pack(side="left", padx=(0, 7))
        preset_buttons = [
            ("⚡ Quick", "QUICK"),
            ("Balanced", "BALANCED"),
            ("Accurate", "ACCURATE"),
        ...
        for label, mode in preset_buttons:
            ttk.Button(
                preset_frame, text=label, style="Preset.TButton",
                command=lambda m=mode: self.set_mode(m),
            ).pack(side="left", padx=2)

        ttk.Label(
            preset_frame, textvariable=self.mode_var, foreground="#666666"
        ).pack(side="left", padx=(10, 0))

        ttk.Label(
            preset_frame,
            text="Human choices are used as evidence; the strategy only decides which comparison to ask next.",
            foreground="#666666",
        ).pack(side="left", padx=(10, 0))
